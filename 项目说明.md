...
| 路径 | 组件 | 说明 |
|------|------|------|
| / | ChatPage | 主对话页面（含登录/注册/对话/历史） |
| /admin | AdminLogin | 管理员登录页 |
| /admin/dashboard | AdminPage | 管理后台仪表盘 |
| /service/:orderNo | ServicePage | 客服服务页面 |

### 2.4 前端组件说明

| 组件 | 文件 | 功能 |
|------|------|------|
| ChatMessage | src/components/ChatMessage.jsx | 聊天消息气泡，支持延迟/Token用量显示 |
| InfoPanel | src/components/InfoPanel.jsx | 左侧信息收集面板，实时展示已收集字段，支持手动修改 |
| AIAnalysisPanel | src/components/AIAnalysisPanel.jsx | AI 深度分析面板（风控推演/证据链/行动计划） |
| AnalysisVisualView | src/components/AnalysisVisualView.jsx | 分析结果可视化视图 |
| AppealTextPanel | src/components/AppealTextPanel.jsx | 申诉文案面板（含进度跟踪 + 反馈闭环） |
| ReportCard | src/components/ReportCard.jsx | 申诉报告卡片展示 |
| TokenPanel | src/components/TokenPanel.jsx | Token 消耗可视化面板 |
| UserCenter | src/components/UserCenter.jsx | 用户中心（余额/充值/用量统计） |
| TypingIndicator | src/components/TypingIndicator.jsx | AI 正在输入动画 |
| ErrorBoundary | src/components/ErrorBoundary.jsx | React 错误边界 |
...
├── ai.js              # 多模型 AI 调用层（对话/提取/评估/扩展）
├── localAI.js         # 本地规则引擎（对话流程+输入验证+报告生成，不消耗Token）
...
└── crypto.js          # AES-256-GCM 加解密 + SHA-256 HMAC 哈希
```
...
│   │   ├── UserCenter.jsx           # 用户中心（余额/充值/用量）
│   │   ├── TypingIndicator.jsx      # AI输入动画
│   │   └── ErrorBoundary.jsx        # 错误边界
│   ├── pages/                       # 页面级组件
│   │   ├── ChatPage.jsx             # 主对话页面（含登录/注册/对话/历史）
│   │   ├── AdminPage.jsx            # 管理后台（仪表盘+会话+用户+设置+进化）
│   │   ├── AdminLogin.jsx           # 管理员登录页
│   │   └── ServicePage.jsx          # 客服服务页面（懒加载）
│   ├── App.jsx                      # 路由配置（React Router 6）
│   ├── main.jsx                     # 应用入口
│   └── index.css                    # 全局样式（TailwindCSS 指令）
├── server/                          # 后端源码（Express + MySQL）
│   ├── index.js                     # 主入口（100+ API路由 + 安全中间件）
│   ├── ai.js                        # 多模型AI调用层
...
├── tailwind.config.js               # TailwindCSS 配置（含微信绿色主题）
