...
export async function extractFieldsWithAI(userMessage, collectedData = {}, currentStep = 0, customApiKey = null, recentHistory = []) {
  ...
  const prompt = `你是信息提取专家。从用户消息中提取商户申诉相关的【具体数据】。
...
## 提取规则
1. 从用户消息中识别能匹配"还需要收集的字段"的【具体数据信息】
2. 用户可能答非所问、说半句话、逻辑混乱、把多个信息混在一起说——你要在混乱中找到有用信息
3. 用户说"被封了"=商户号封禁，"不能收款"=支付权限关闭，"钱取不出来"=资金冻结
4. 用户说"没有"/"不知道"对于投诉/退款/申诉历史是有效回答（分别对应：没有投诉/暂无退款政策/首次申诉）
5. 如果用户同时提到了多个字段的信息，全部提取
6. 只提取你有把握的信息，不确定的不要提取
7. 不要编造用户没说的信息
8. 对于已收集的字段，如果用户明显在纠正（如"不是XX，是YY"），提取纠正后的值

## 【严格过滤规则 — 防止误提取】
⛔ 以下内容绝对不是字段数据，严禁提取：
- 用户的提问（如"怎么解封""还有什么问题吗""下一步怎么做"）
- 用户的催促/抱怨（如"快点""为什么这么慢""搞不懂"）
- 用户的闲聊/确认（如"好的""明白了""没问题""可以"）
- 用户在复述AI的话或讨论流程

⛔ 字段值必须是【具体的事实数据】，不能是用户的疑问句或感叹句！
例如：
- "没什么问题了吗，怎么解封告诉我呀" → 这是提问，不是任何字段的值，extracted应为{}
- "工商银行" → 这是开户银行的具体数据，可以提取
- "1409570162" → 这是商户号的具体数据，可以提取

⛔ bank_name（开户银行）的值只能是银行名称（如工商银行、建设银行等），不能是用户的提问或其他文字
⛔ 如果用户消息只是在提问、催促或闲聊，没有包含任何具体数据信息，extracted必须为空对象{}

返回严格JSON格式：
{"extracted":{"字段key":"提取的值"},"intent":"用简短一句话描述用户意图","correction":false}
correction为true表示用户在纠正之前的信息。如果没有可提取的信息，extracted必须为空对象{}。`
